...
// Session config for Render HTTPS
app.use(session({
...
// Login - explicitly save session before response
...
app.get('/api/user', asyncHandler(async (req, res) => {
  console.log('/api/user called, session:', req.session?.username);
  if (!req.session || !req.session.username) {
...
app.post('/api/posts/:id/favorite', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (idx > -1) post.favorites.splice(idx, 1);
  else post.favorites.push(req.session.username);
  await writePosts(posts);
...
app.post('/api/posts/:id/comment', requireAuth, asyncHandler(async (req, res) => {
  const { text } = req.body;
  if (!text || text.trim().length === 0) return res.status(400).json({ message: 'Комментарий не может быть пустым' });
  const posts = await readPosts();
  const post = posts.find(p => p.id === parseInt(req.params.id));
  if (!post) return res.status(404).json({ message: 'Пост не найден' });
  post.comments = post.comments || [];
  post.comments.push({ id: Date.now(), author: req.session.username, text: text.trim(), timestamp: new Date().toISOString() });
  await writePosts(posts);
...
app.post('/api/posts/:id/repost', requireAuth, asyncHandler(async (req, res) => {
  ...
  const repost = { id: Date.now(), author: req.session.username, text: `Репост от @${post.author}: ${post.text}`, image: post.image, timestamp: new Date().toISOString(), likes: [], favorites: [], comments: [], shares: 0, reposts: 0, originalAuthor: post.author, originalPostId: post.id };
...
app.get('/api/blacklist', requireAuth, asyncHandler(async (req, res) => {
  ...
  const users = await readUsers();
  const blacklistedUsers = users.filter(u => blacklist.includes(u.username)).map(u => ({ username: u.username, avatar: u.avatar, avatarImage: u.avatarImage || null }));
...
app.delete('/api/posts/:id', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (posts[index].author !== req.session.username) return res.status(403).json({ message: 'Вы не можете удалить чужой пост' });
...
function getDmContacts(username, messages) {
  const userMessages = messages.filter(m => !m.groupId && (m.from === username || m.to === username));
  const grouped = {};
  ...
  return Object.entries(grouped).map(([name, msgs]) => ({ id: name, type: 'dm', name, displayName: `@${name}`, lastMessage: msgs[msgs.length - 1]?.text, timestamp: msgs[msgs.length - 1]?.timestamp, unread: msgs.some(m => m.to === username && !m.read) }));
}
...
app.delete('/api/messages/:id', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (msgIndex === -1) return res.status(404).json({ message: 'Сообщение не найдено' });
  const msg = messages[msgIndex];
  const isDm = !msg.groupId;
  const isReceiver = msg.to === req.session.username;
  const isSender = msg.from === req.session.username;
  if (isDm) {
    if (!isSender && !isReceiver) return res.status(403).json({ message: 'Вы не участник этого чата' });
  } else {
    const group = findGroupById(await readGroups(), msg.groupId);
    if (!group || !isGroupMember(group, req.session.username)) return res.status(403).json({ message: 'Вы не участник этой группы' });
  }
...
app.delete('/api/chat/:username', requireAuth, asyncHandler(async (req, res) => {
  ...
  const filtered = messages.filter(m => {
    ...
    return !((m.from === req.session.username && m.to === username) || (m.from === username && m.to === req.session.username));
  });
...
app.get('/api/messages', requireAuth, asyncHandler(async (req, res) => {
  ...
  const blacklist = me?.blacklist || [];
  const dmContacts = getDmContacts(username, messages).filter(c => !blacklist.includes(c.name));
  const groupContacts = groups.filter(g => isGroupMember(g, username)).map(g => {
    const msgs = messages.filter(m => m.groupId === g.id);
    const last = msgs[msgs.length - 1];
    return { id: g.id, type: g.type, name: g.id, displayName: g.type === 'channel' ? `📢 ${g.title}` : `👥 ${g.title}`, slug: g.slug, lastMessage: last?.text, timestamp: last?.timestamp, unread: msgs.some(m => m.to === `group:${g.id}` && m.from !== username && !m.read) };
  });
  const contacts = [...dmContacts, ...groupContacts].sort((a, b) => { const ta = a.timestamp ? new Date(a.timestamp).getTime() : 0; const tb = b.timestamp ? new Date(b.timestamp).getTime() : 0; return tb - ta; });
  res.json({ contacts });
}));
...
app.get('/api/chat/dm/:username', requireAuth, asyncHandler(async (req, res) => {
  ...
  const users = await readUsers();
  if (!findUserByUsername(users, otherUser)) return res.status(404).json({ message: 'Пользователь не найден' });
  const messages = await readMessages();
  const chat = messages.filter(m => !m.groupId && ((m.from === req.session.username && m.to === otherUser) || (m.from === otherUser && m.to === req.session.username))).sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
  chat.forEach(msg => { if (msg.to === req.session.username && !msg.read) msg.read = true; });
  await writeMessages(messages);
...
app.get('/api/chat/group/:groupId', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!group || !isGroupMember(group, req.session.username)) return res.status(404).json({ message: 'Группа не найдена' });
  const messages = await readMessages();
  const chat = messages.filter(m => m.groupId === group.id).sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
  chat.forEach(msg => { if (msg.to === `group:${group.id}` && msg.from !== req.session.username && !msg.read) msg.read = true; });
  await writeMessages(messages);
...
app.post('/api/messages/send', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!text && !media && !voice) return res.status(400).json({ message: 'Пустое сообщение' });
  const bodyText = String(text || '').trim();
  if (!bodyText && !media && !voice) return res.status(400).json({ message: 'Пустое сообщение' });
  const messages = await readMessages();
  if (groupId) {
    const group = findGroupById(await readGroups(), groupId);
    if (!group || !isGroupMember(group, req.session.username)) return res.status(404).json({ message: 'Группа не найдена' });
    if (!canPostInGroup(req.session.username, group)) return res.status(403).json({ message: 'В канале писать могут только администраторы' });
    const newMessage = { id: Date.now(), from: req.session.username, to: `group:${group.id}`, groupId: group.id, text: bodyText || null, media: media || null, mediaType: mediaType || null, voice: voice || null, timestamp: new Date().toISOString(), read: false, reactions: {}, deleted: false };
  ...
  if (isBlacklisted(req.session.username, recipient)) return res.status(403).json({ message: 'Вы в чёрном списке у этого пользователя' });
  if (!canMessageUser(req.session.username, recipient)) return res.status(403).json({ message: 'Этот пользователь не принимает сообщения' });
  const newMessage = { id: Date.now(), from: req.session.username, to: recipient.username, groupId: null, text: bodyText || null, media: media || null, mediaType: mediaType || null, voice: voice || null, timestamp: new Date().toISOString(), read: false, reactions: {}, deleted: false };
  messages.push(newMessage);
  await writeMessages(messages);
...
app.post('/api/messages/:id/react', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (msg.groupId) {
    const group = findGroupById(await readGroups(), msg.groupId);
    if (!group || !isGroupMember(group, req.session.username)) return res.status(403).json({ message: 'Вы не участник этого чата' });
...
app.get('/api/groups', requireAuth, asyncHandler(async (req, res) => {
  ...
  const groups = allGroups.filter(g => isGroupMember(g, req.session.username)).map(g => ({ id: g.id, title: g.title, type: g.type, slug: g.slug, owner: g.owner, membersCount: g.members?.length || 0 }));
  res.json({ groups });
}));

app.post('/api/groups/create', requireAuth, asyncHandler(async (req, res) => {
  const { title, type, slug, members } = req.body;
  if (!title || !String(title).trim()) return res.status(400).json({ message: 'Укажите название' });
  const groupType = type === 'channel' ? 'channel' : 'group';
  let groupSlug = null;
  if (slug) {
    groupSlug = normalizeUsername(slug);
    if (!GROUP_SLUG_REGEX.test(groupSlug)) return res.status(400).json({ message: 'Slug: a-z, 0-9, _, минимум 5 символов' });
    if ((await readGroups()).some(g => g.slug === groupSlug)) return res.status(409).json({ message: 'Такой slug уже занят' });
  }
  const groups = await readGroups();
  const id = `g_${Date.now()}`;
  const owner = req.session.username;
  const memberSet = new Set([owner, ...(Array.isArray(members) ? members : [])].map(normalizeUsername));
  const newGroup = { id, type: groupType, title: String(title).trim().slice(0, 80), slug: groupSlug, owner, admins: [owner], members: [...memberSet], createdAt: new Date().toISOString() };
...
app.post('/api/groups/:id/join', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (group.type === 'channel') return res.status(400).json({ message: 'В канал можно только по приглашению' });
  if (!isGroupMember(group, req.session.username)) { group.members.push(req.session.username); await writeGroups(groups); }
...
app.post('/api/groups/:id/invite', requireAuth, asyncHandler(async (req, res) => {
  ...
  const group = findGroupById(groups, req.params.id);
  if (!group) return res.status(404).json({ message: 'Не найдено' });
  if (group.owner !== req.session.username && !group.admins?.includes(req.session.username)) return res.status(403).json({ message: 'Нет прав' });
  const user = await store.getUser(username);
  if (!user) return res.status(404).json({ message: 'Пользователь не найден' });
  if (!isGroupMember(group, user.username)) { group.members.push(user.username); await writeGroups(groups); }
...
app.get('/api/profile', requireAuth, asyncHandler(async (req, res) => {
  ...
  res.json({ username: req.session.username, avatar: user?.avatar || '👤', avatarImage: user?.avatarImage || null, bio: user?.bio || 'О себе ничего не рассказано', followers: user?.followers?.length || 0, following: user?.following?.length || 0, posts: posts.length, settings: user?.settings || defaultSettings() });
}));

app.post('/api/profile/avatar', requireAuth, asyncHandler(async (req, res) => {
  const { avatarImage } = req.body;
  if (!avatarImage) return res.status(400).json({ message: 'Изображение не предоставлено' });
  if (!avatarImage.startsWith('data:image/')) return res.status(400).json({ message: 'Некорректный формат изображения' });
  const user = await store.getUser(req.session.username);
  if (!user) return res.status(404).json({ message: 'Пользователь не найден' });
...
app.post('/api/settings/change-password', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!oldPassword || !newPassword) return res.status(400).json({ message: 'Укажите текущий и новый пароль' });
  if (newPassword.length < 5) return res.status(400).json({ message: 'Новый пароль: минимум 5 символов' });
  const user = await store.getUser(req.session.username);
  if (!user) return res.status(404).json({ message: 'Пользователь не найден' });
  if (user.password !== oldPassword) return res.status(400).json({ message: 'Неверный текущий пароль' });
...
app.post('/api/settings/update-email', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (emailTaken) return res.status(409).json({ message: 'Этот email уже привязан к другому аккаунту' });
  const user = await store.getUser(req.session.username);
  if (!user) return res.status(404).json({ message: 'Пользователь не найден' });
  user.email = email;
  await store.saveUser(user);
...
app.post('/api/settings/change-username', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (validation.username === req.session.username) return res.status(400).json({ message: 'Username не изменился' });
  const users = await readUsers();
  if (isUsernameTaken(users, validation.username)) return res.status(409).json({ message: 'Этот @username уже занят' });
...
app.put('/api/settings', requireAuth, asyncHandler(async (req, res) => {
  ...
  user.settings = { ...defaultSettings(), ...user.settings, ...incoming, notifications: { ...defaultSettings().notifications, ...user.settings.notifications, ...incoming.notifications }, privacy: { ...defaultSettings().privacy, ...user.settings.privacy, ...incoming.privacy } };
...
app.patch('/api/profile', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (req.body.username !== undefined) {
    ...
    if (!validation.valid) return res.status(400).json({ message: validation.message });
    if (isUsernameTaken(users, validation.username, user.username)) return res.status(409).json({ message: 'Этот @username уже занят' });
    const oldUsername = user.username;
    if (oldUsername !== validation.username) { await store.renameUsername(oldUsername, validation.username); req.session.username = validation.username; user.username = validation.username; }
  }
  await store.saveUser(user);
  res.json({ message: 'Профиль обновлён', username: user.username, bio: user.bio });
}));
...
app.get('/api/account/stats', requireAuth, asyncHandler(async (req, res) => {
  ...
  res.json({ messagesCount: messages.length, postsCount: posts.length, cacheHint: 'Клиентские настройки темы и языка хранятся в браузере и на сервере' });
}));
...
app.get('/api/users/search', requireAuth, asyncHandler(async (req, res) => {
  ...
  const users = (await readUsers()).filter(u => { if (u.username === req.session.username) return false; if (!query) return true; return u.username.includes(query) || u.username === query; });
  const sorted = users.sort((a, b) => { const aExact = a.username === query ? 0 : 1; const bExact = b.username === query ? 0 : 1; if (aExact !== bExact) return aExact - bExact; return a.username.localeCompare(b.username); });
  const usersPayload = await Promise.all(sorted.filter(u => query.length >= 1 || canViewProfile(req.session.username, u)).slice(0, 30).map(u => publicUserPayload(u, req.session.username)));
...
app.get('/api/stand/feed', requireAuth, asyncHandler(async (req, res) => {
  ...
  const stands = (await Promise.all(standsRaw.map(s => enrichStand(s, req.session.username)))).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
...
app.post('/api/stand/create', requireAuth, asyncHandler(async (req, res) => {
  ...
  const newStand = { id: Date.now(), author: req.session.username, video, caption: String(caption || '').trim().slice(0, 300), timestamp: new Date().toISOString(), likes: [], favorites: [], comments: [], shares: 0, reposts: 0 };
...
app.post('/api/stand/:id/like', requireAuth, asyncHandler(async (req, res) => {
  const stand = await standAction(req.params.id, (s) => { s.likes = s.likes || []; const i = s.likes.indexOf(req.session.username); if (i > -1) s.likes.splice(i, 1); else s.likes.push(req.session.username); });
...
app.post('/api/stand/:id/favorite', requireAuth, asyncHandler(async (req, res) => {
  const stand = await standAction(req.params.id, (s) => { s.favorites = s.favorites || []; const i = s.favorites.indexOf(req.session.username); if (i > -1) s.favorites.splice(i, 1); else s.favorites.push(req.session.username); });
  if (!stand) return res.status(404).json({ message: 'Не найдено' });
...
app.post('/api/stand/:id/comment', requireAuth, asyncHandler(async (req, res) => {
  ...
  const stand = await standAction(req.params.id, (s) => { s.comments = s.comments || []; s.comments.push({ id: Date.now(), author: req.session.username, text: text.trim(), timestamp: new Date().toISOString() }); });
  if (!stand) return res.status(404).json({ message: 'Не найдено' });
...
app.post('/api/stand/:id/repost', requireAuth, asyncHandler(async (req, res) => {
  ...
  const repost = { id: Date.now(), author: req.session.username, video: original.video, caption: `Repost @${original.author}: ${original.caption || ''}`, timestamp: new Date().toISOString(), likes: [], favorites: [], comments: [], shares: 0, reposts: 0, originalAuthor: original.author, originalStandId: original.id };
  stands.push(repost);
  await writeStands(stands);
...
app.delete('/api/stand/:id', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (stands[idx].author !== req.session.username) return res.status(403).json({ message: 'Можно удалить только своё видео' });
  stands.splice(idx, 1);
  await writeStands(stands);
...
app.post('/api/voice/send', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!to || !audioData) return res.status(400).json({ message: 'Укажите получателя и аудио' });
  const users = await readUsers();
  if (!users.find(u => u.username === to)) return res.status(404).json({ message: 'Пользователь не найден' });
  const newVoiceMsg = { id: Date.now(), from: req.session.username, to: to, audioData: audioData, timestamp: new Date().toISOString(), read: false };
...
app.post('/api/stories/create', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!media) return res.status(400).json({ message: 'Укажите контент истории' });
  const stories = await readStories();
  const newStory = { id: Date.now(), author: req.session.username, media: media, duration: duration || 10, timestamp: new Date().toISOString(), views: [] };
...
app.get('/api/stories/feed', requireAuth, asyncHandler(async (req, res) => {
  ...
  const enrichedStories = stories.filter(s => currentUser?.following?.includes(s.author) || s.author === req.session.username).map(s => { const user = users.find(u => u.username === s.author); return { ...s, avatar: user?.avatar || '👤', avatarImage: user?.avatarImage || null }; }).sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp));
...
async function start() {
  ...
  if (info.mode === 'postgres') {
    try {
      const counts = await Promise.all([store.readUsers().then(u => u.length), store.readPosts().then(p => p.length), store.readMessages().then(m => m.length), store.readStands().then(s => s.length)]);
  ...
  function sendToUser(targetUser, data) { const clients = wsClients.get(targetUser); if (clients) { const payload = JSON.stringify(data); clients.forEach(c => { if (c.readyState === WebSocket.OPEN) c.send(payload); }); } }
