...
async function saveUserPg(user) {
  await db.query(
    `INSERT INTO users (username, password, avatar, bio, settings, created_at)
...
async function writeMessagesPg(messages) {
  ...
  for (const m of messages) {
    await db.query(
      `INSERT INTO messages (id, from_user, to_user, group_id, body, timestamp, read)
       VALUES ($1,$2,$3,$4,$5,$6,$7) ON CONFLICT (id) DO NOTHING`,
      [m.id, m.from, m.to, m.groupId || null, m.text, m.timestamp, Boolean(m.read)]
    );
  }
}
